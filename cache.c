...
//  printf("length of Table %d\n", lengthOfTable);
 // printf("numOfSets %d\n", numOfSets);
  Addr currentAddress=lineaddr>>numOfSets;
 ...
 // printf("numOfSets %d\n", numOfSets);
  ...
  while(i<lengthOfTable){
	...
	if(c->sets[sixBits].line[i].tag==0){
        ...
        c->sets[sixBits].line[i].last_access_time=cycle_count;
        c->sets[sixBits].line[i].tag=currentAddress;
  ...
  if(done==FALSE){
          if(c->repl_policy){
                  ...
                  c->sets[sixBits].line[random_num].tag=currentAddress;
                  ...
                  while(i<lengthOfTable){
                    ...
                    c->sets[sixBits].line[cacheBlock].tag=currentAddress;
