...
void decode_instruction(){
  ...
  /*** You need to fill the rest of the decoded fields ****/
  ...
  INST.D               = get_bits( INST.IR, 5, 4);
  INST.amount4         = get_bits( INST.IR, 3, 0);
  INST.trapvect8       = get_bits( INST.IR, 7, 0);
...
/************************************************************/
 ...
 void execute_ADD(){
	int addition_value;
	if(get_bits(INST.IR,5,5)==0){
		addition_value=NEXT_LATCHES.REGS[INST.SR1]+NEXT_LATCHES.REGS[INST.SR2];
		NEXT_LATCHES.REGS[INST.DR]=Low16bits(addition_value);
		}
	else{
		addition_value=NEXT_LATCHES.REGS[INST.SR1]+SEXT(INST.imm5,5);
		NEXT_LATCHES.REGS[INST.DR]=Low16bits(addition_value);
		
		}
	setcc(Low16bits(addition_value));
 }


 void execute_BR(){
	if ((INST.n & NEXT_LATCHES.N)|(INST.z & NEXT_LATCHES.Z)|(INST.p & NEXT_LATCHES.P)){
		NEXT_LATCHES.PC=Low16bits(NEXT_LATCHES.PC+LSHF(Low16bits(SEXT(INST.PCoffset9,9)),1));
		}
	
 }

 void execute_LDB(){
	int offset_addr=NEXT_LATCHES.REGS[INST.BaseR]+SEXT(INST.boffset6,6);
	int value=SEXT(read_byte(Low16bits(offset_addr)),8);
	NEXT_LATCHES.REGS[INST.DR]=Low16bits(value);
	setcc(Low16bits(value));
 }


 void execute_STB(){
	int mem_address=NEXT_LATCHES.REGS[INST.BaseR]+SEXT(INST.boffset6,6);
	int seven_bits=get_bits(NEXT_LATCHES.REGS[INST.DR],7,0);
	write_byte(Low16bits(mem_address), seven_bits);
 }
 
 
 void execute_JSR(){
	int temp=CURRENT_LATCHES.REGS[get_bits(INST.IR,8,6)];
	NEXT_LATCHES.REGS[7] = NEXT_LATCHES.PC;
	if(INST.B==0){
		NEXT_LATCHES.PC=Low16bits(temp);
		}
	else{
		NEXT_LATCHES.PC=Low16bits(NEXT_LATCHES.PC+LSHF(SEXT(INST.PCoffset11,11),1));
	}
 }
 ...
 void execute_AND(){
	int value;
	if(get_bits(INST.IR,5,5)==0){
		value=NEXT_LATCHES.REGS[INST.SR1] & NEXT_LATCHES.REGS[INST.SR2];
		NEXT_LATCHES.REGS[INST.DR]=Low16bits(value);
		}
	else{
		value=NEXT_LATCHES.REGS[INST.SR1] & SEXT(INST.imm5,5);
		NEXT_LATCHES.REGS[INST.DR]=Low16bits(value);
	}
 ...
 void execute_LDW(){
	int offset_addr=NEXT_LATCHES.REGS[INST.BaseR]+LSHF(SEXT(INST.offset6,6),1);
	int value=read_word(Low16bits(offset_addr));
	NEXT_LATCHES.REGS[INST.DR]=Low16bits(value);
	setcc(Low16bits(value));
 }
 ...
 void execute_STW(){
	int mem_address=NEXT_LATCHES.REGS[INST.BaseR]+LSHF(SEXT(INST.offset6,6),1);
	write_word(Low16bits(mem_address),Low16bits(NEXT_LATCHES.REGS[INST.DR]));
 }
 ...
 void execute_JMP(){
	NEXT_LATCHES.PC=Low16bits(NEXT_LATCHES.REGS[INST.BaseR]);
 }


 void execute_SHF(){
	int shift_value;
	if(get_bits(INST.IR,4,4)==0){
		shift_value=LSHF(NEXT_LATCHES.REGS[INST.SR1],INST.amount4);
		NEXT_LATCHES.REGS[INST.DR]=Low16bits(shift_value);
		}
	else{
		if(get_bits(INST.IR,5,5)==0){
			shift_value=RSHF(NEXT_LATCHES.REGS[INST.SR1], INST.amount4,0);
			NEXT_LATCHES.REGS[INST.DR]=Low16bits(shift_value);}
		else{
			int fifteen_bit=get_bits(NEXT_LATCHES.REGS[INST.SR1],15,15);
			shift_value=RSHF(NEXT_LATCHES.REGS[INST.SR1], INST.amount4,fifteen_bit);
			NEXT_LATCHES.REGS[INST.DR]=Low16bits(shift_value);}
		}
	setcc(Low16bits(shift_value));
 }
 
 
 void execute_LEA(){
	int value=NEXT_LATCHES.PC+LSHF(SEXT(INST.PCoffset9,9),1);
	NEXT_LATCHES.REGS[INST.DR]=Low16bits(value);
