...
uns64 memsys_access_modeBC(Memsys *sys, Addr lineaddr, Access_Type type){
  ...
  if(type == ACCESS_TYPE_STORE){
   ...
   if(needs_icache_access){
    Flag outcome=cache_access(sys->icache, lineaddr, mark_dirty);
    if(outcome==MISS){
   ...
   if(needs_dcache_access){
             ...
             Flag outcome=cache_access(sys->dcache, lineaddr, mark_dirty);
            if(outcome==MISS){
                  globalWrite=mark_dirty;
                  delay+=memsys_L2_access(sys, lineaddr,FALSE);  // at this point you had a miss in L1. Doesn't matter if it was a read or write because you need to go to see if its in l2
                  cache_install(sys->dcache, lineaddr, mark_dirty);
                  if(sys->dcache->last_evicted_line.dirty){
                        memsys_L2_access(sys, sys->dcache->last_evicted_line.tag, TRUE);
  ...
  if(outcome==MISS){
            ...
            if(sys->l2cache->last_evicted_line.dirty ){
                ...
                sys->l2cache->last_evicted_line.dirty=FALSE;
                    //cache_install(sys->dram,lineaddr,is_writeback);
            }
        }
